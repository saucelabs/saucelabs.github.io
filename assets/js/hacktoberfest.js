...
(function hacktoberfestStats() {
  const clientWithAuth = new window.Octokit.Octokit({
    auth: PUBLIC_READ_ONLY_TOKEN,
  });
  const orgsAndRepos = PARTICIPATING_GH_ORGS.map((orgName) =>
    clientWithAuth.paginate('GET /orgs/:org/repos', {
      org: orgName,
      type: 'sources',
    })
  );
  ...
  const totalOpenPullRequests = document.getElementById(
    'total-open-pull-requests'
  );
  const totalClosedIssues = document.getElementById('total-closed-issues');
  const totalClosedPullRequests = document.getElementById(
    'total-closed-pull-requests'
  );
  const contributorsList = document.getElementById(
  ...
  function formatIssue(issue) {
    const orgAndRepo = issue.repository_url.replace(
  ...
  function build(org) {
    return org
      .then((orgRepos) =>
        orgRepos.map((repo) =>
          clientWithAuth.paginate('GET /repos/:owner/:repo/issues', {
          ...
          values.forEach((v) => {
            ...
            v.forEach((issue) => {
              if (
                issue.pull_request &&
                !issue.closed_at &&
                issue.created_at >= '2022-10-01'
              ) {
                openPrs.push(issue);
                contributors.push(formatContributor(issue.user));
              } else if (
                issue.pull_request &&
                issue.closed_at >= '2022-10-01' &&
                issue.closed_at <= '2022-10-31'
              ) {
                closedPrs += 1;
                contributors.push(formatContributor(issue.user));
              } else if (
                ...
                document.getElementById('hacktoberfest-issues').innerHTML +=
                  formatIssue(issue);
              }
            });
            participatingRepos.push(v[0].repository_url);
            document.getElementById('hacktoberfest-repos').innerHTML +=
              formatRepo(v[0].repository_url);
          });
  ...
  const loadingPromises = Object.keys(orgsAndRepos).map((org) =>
    build(orgsAndRepos[org])
  );
