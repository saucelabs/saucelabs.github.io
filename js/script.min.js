...
$(document).ready(function(){const basePath=window.location.pathname.split('/')[1]
const navigation=$('.navigation')
const homePageLogo=$('#home-page-logo')
const contentPageLogo=$('#content-page-logo')
const navbar=$('.navbar')
if(basePath.length>0){homePageLogo.remove()
...
navbar.addClass('content-page')}else{homePageLogo.removeClass('invisible')
contentPageLogo.remove()}});(function($){'use strict'
const NAVIGATION_MENU_TRESHOLD=100
const navigation=$('.navigation')
$(window).scroll(function(){if(navigation.offset().top>NAVIGATION_MENU_TRESHOLD){navigation.addClass('nav-bg')}else{navigation.removeClass('nav-bg')}})
$('.navbar-toggler').click(()=>{if(navigation.offset().top<=NAVIGATION_MENU_TRESHOLD){navigation.toggleClass('nav-bg')}})
$('[data-background]').each(function(){$(this).css({'background-image':'url('+$(this).data('background')+')'})})
$('[data-color]').each(function(){$(this).css({'background-color':$(this).data('color')})})
$('[data-progress]').each(function(){$(this).css({'bottom':$(this).data('progress')})})
window.onload=function(){const parallaxBox=document.getElementById('parallax')
...
function setElementPosition(){c1left=l1.offsetLeft
c1top=l1.offsetTop
...
c8top=l8.offsetTop}
function clearElementPosition(){l1.removeAttribute('style')
...
function moveElements(event){event=event||window.event
...
setElementPosition()
window.onresize=function(){clearElementPosition()
setElementPosition()
moveElements()}
parallaxBox.onmousemove=moveElements}
function mouseParallax(obj,left,top,mouseX,mouseY,speed){const parentObj=obj.parentNode
...
$('.testimonial-slider').slick({dots:true,infinite:true,speed:300,slidesToShow:1,arrows:false,adaptiveHeight:true})
$('.client-logo-slider').slick({infinite:true,slidesToShow:5,slidesToScroll:1,autoplay:true,dots:false,arrows:false,responsive:[{breakpoint:1024,settings:{slidesToShow:3,slidesToScroll:1}},{breakpoint:600,settings:{slidesToShow:3,slidesToScroll:1}},{breakpoint:480,settings:{slidesToShow:2,slidesToScroll:1}},{breakpoint:400,settings:{slidesToShow:1,slidesToScroll:1}}]})
var containerEl=document.querySelector('.shuffle-wrapper')
if(containerEl){var Shuffle=window.Shuffle
var myShuffle=new Shuffle(document.querySelector('.shuffle-wrapper'),{itemSelector:'.shuffle-item',buffer:1})
window.jQuery('input[name="shuffle-filter"]').on('change',function(evt){var input=evt.currentTarget
if(input.checked){myShuffle.filter(input.value)}})}})(window.jQuery)